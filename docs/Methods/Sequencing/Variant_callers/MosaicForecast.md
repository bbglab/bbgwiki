...
1. We have created a docker image with all dependencies installed: <https://hub.docker.com/r/yanmei/mosaicforecast>  
  ...
  !!! note
        ...
        After attaching your local MF directory to the docker image, you would be able to read
        and write from that directory in your docker image. The attached directory in the docker image would be "/MF".
...
3. If you trained model with refined-genotypes (mosaic, het, refhom, repeat), then the "model\_type" is "Refine";
  otherwise if you trained model with Phasing (hap=2, hap=3, hap>3), then the "model\_type" is "Phase".
4. We also added annotations of additional filtrations: Predicted mosaics with extra-high read depths (>=2X),
  sites with >=1.5X read depths and >=20% AF were marked as "low-confidence"; predicted mosaics with only one alt
  allele and <1% AF were marked as "cautious".  

> You may use our models trained with brain WGS data for SNPs (paired-end read at 50-250X read depths, we train our
> models based on Mutect2-PON callings. To our experience, the models were pretty robust across different depths, but
> the best strategy would be using a model with similar depth with your data):
>
